...
params.n_frames = 10000;
params.sigma = 0.0001;
params.shuffle = 1001;
params.get_data = 1;
[learn_feats, learn_magspecs, n_f_learn, ref_learn_data] = load_data(params);
...
[test_feats, test_magspecs, n_f_test, ref_t_data] = load_data(params);

%%  Trying a nadaraya-watson

methods_to_try = [1,4,5];
...
for methodix=1:length(methods_to_try)
    ...
    [res_struct{methodix}.m, res_struct{methodix}.err] = nadaraya_watson(learn_feats, learn_magspecs, test_feats, test_magspecs, @covariance,method,1,50);    
    disp(['mean l2 error of  ' num2str(mean(res_struct{methodix}.err))]);
    
    % sliding median filtering ?
    res_struct{methodix}.m_filt = medfilt1(res_struct{methodix}.m',5)';
    
    % reconstruction
    init_vec = randn(n_f_test*hoptime*sr,1);
    [res_struct{methodix}.x_rec] = gl_reconstruct(res_struct{methodix}.m_filt, init_vec, sr, 10, wintime*sr, hoptime*sr);
