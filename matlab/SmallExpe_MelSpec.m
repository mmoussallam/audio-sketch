...
[learn_magspecs, learn_feats, ref_learn_data] = load_data_one_file_melspec(learn_file, 32000, 0.001, params);
...
[test_magspecs, test_feats, ref_t_data] = load_data_one_file_melspec(test_file, 32000, 0.001, params);
...
%     
    ...
    res_struct{methodix}.m_filt = [res_struct{methodix}.m ; flipud(res_struct{methodix}.m(2:end-1,:))];
...
parameters
sr =32000;
[ellis_mfcc,~,~] = melfcc(ref_t_data, sr,'maxfreq',maxfreq, ...
                                'wintime',wintime,'hoptime',hoptime, ...
                                'nbands',nbands,'numcep',numcep); 

[x_rec_ellis,~,~] = invmelfcc(ellis_mfcc, sr,'maxfreq',maxfreq, ...
                                'wintime',wintime,'hoptime',hoptime, ...
                                'nbands',nbands,'numcep',numcep); 
yaafeopts.win_size = params.wintime*params.sr;
yaafeopts.step_size = params.hoptime*params.sr;
yaafeopts.fs = params.sr;
secondyaafe_df = get_yaafeFeat({'magspec'}, yaafeopts);

FeatStruct = secondyaafe_df.process(x_rec_ellis');                                   
ellis_spec = FeatStruct.magspec.data;        

L = min(length(ref_t_data), length(x_rec_ellis));
bss_eval_sources_nosort(ref_t_data(1:L)', x_rec_ellis(1:L)')


Lnous = min(length(ref_t_data), length(res_struct{methodix}.x_rec));
bss_eval_sources_nosort(ref_t_data(1:Lnous)', res_struct{methodix}.x_rec(1:Lnous)')

figure
subplot(211)
imagesc(log(ellis_spec))
subplot(212)
imagesc(log(test_magspecs))
