...
#        sizes.append(float(D['size'])/(1024.0*1024.0))
    ...
    print cons_scores
plt.plot(np.array(sparsities), 100*np.array(cons_scores),'o-', linewidth=1.5)
plt.xlabel('Sparsity $k$',fontsize=16)
plt.ylabel('Recognition rate (%)',fontsize=16)
    
legends.append(("C10 ($\lambda_H = $%d)"%0))
...
#        sizes.append(float(D['size'])/(1024.0*1024.0))
        ...
        sizes.append(D['size'][0][0])
        print cons_scores
    plt.plot(np.array(sparsities), 100*np.array(cons_scores),'s--', linewidth=(il+1.5))
#    plt.xlabel('Sparsity $k$',fontsize=16)
#    plt.ylabel('Recognition rate (%)',fontsize=16)
        
    legends.append(("$\lambda_H = $%d - $b$"%l))
